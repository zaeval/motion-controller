...
struct ActionEvaluatorTests {
    ...
    private func reading(
        _ pose: StaticPose? = nil, still: Bool = true, steps: [PinchAxisControl.Step] = [], at time: TimeInterval
    ) -> GestureReading {
        GestureReading(
            ...
            openness: nil, palmFacesCamera: true, extendedFingers: [], steps: steps,
    ...
    @Test func aHeldOpenPalmPlaysAndPausesOnceButOnlyPastTheParkingGesturesHold() {
        var evaluator = ActionEvaluator()
        // A parking gesture shows the palm for 0.6–0.9 s: no action yet.
        #expect(feed(&evaluator, from: 0, seconds: 0.9) { reading(.openPalm, at: $0) }.isEmpty)
        #expect(feed(&evaluator, from: 0.9, seconds: 0.5) { reading(.openPalm, at: $0) } == [.media(.playPause)])
        // Held on: the cooldown and the release keep it from firing again.
        #expect(feed(&evaluator, from: 1.4, seconds: 3.0) { reading(.openPalm, at: $0) }.isEmpty)
        #expect(feed(&evaluator, from: 4.4, seconds: 0.3) { reading(.pointIndex, at: $0) }.isEmpty)
        #expect(feed(&evaluator, from: 4.7, seconds: 1.5) { reading(.openPalm, at: $0) } == [.media(.playPause)])
    }
    ...
    @Test func aMovingHandNeverFinishesAHold() {
        ...
        #expect(feed(&evaluator, from: 3.0, seconds: 2.0) { reading(.threeFingers, still: false, at: $0) }.isEmpty)
    }

    @Test func threeFingersSwitchAppsAndTheBackOfTheHandDoesNothing() {
        var evaluator = ActionEvaluator()
        #expect(feed(&evaluator, from: 0, seconds: 0.7) { reading(.threeFingers, at: $0) } == [.keyCombo(.commandTab)])
    ...
    @Test func aPoseRightAfterASwipeIsJustTheHandComingBack() {
        ...
        #expect(feed(&evaluator, from: 0.03, seconds: 0.9) { reading(.threeFingers, at: $0) }.isEmpty)
        // Held on well past the swipe, it is meant.
        #expect(feed(&evaluator, from: 0.93, seconds: 0.9) { reading(.threeFingers, at: $0) } == [.keyCombo(.commandTab)])
    }
