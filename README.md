...
확대/축소는 macOS의 화면 확대 단축키(⌥⌘= / ⌥⌘-)를 보낸다. 그 단축키는 **기본적으로 꺼져 있어서**
(`com.apple.universalaccess`의 `closeViewHotkeysEnabled = 0`) 아무 일도 일어나지 않는데, 꺼져 있으면
앱 확대(⌘+ / ⌘-)로 대신 보낸다. 화면 전체를 확대하고 싶으면 시스템 설정 > 손쉬운 사용 > 확대/축소에서
"키보드 단축키로 확대/축소 사용"을 켜면 된다.
