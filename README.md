...
아무도 없다가 사람이 나타나면 그 사람부터 확인한다. 3초 넘게 비어 있던 자리에 누가 오면 10초 동안 얼굴을 보고,
등록되지 않은 얼굴이 또렷하게 네 번 연속 (약 1초) 잡히면 10초를 기다리지 않고 바로 화면을 까맣게 하고 잠근다
(사용자 요청 2026-09-16). 등록된 얼굴이 보이면 확인을 멈추고, 얼굴이 안 보이거나 너무 멀거나 방이 어두우면
잠그지 않는다. 얼굴이 잘 안 잡혀 잠긴 경우에도 Touch ID·암호로 바로 풀 수 있다.
...
- 사람이 계속 있던 자리에 다른 사람이 끼어드는 건 막지 않는다. 빈 자리에 새로 나타난 얼굴만 확인한다.
