...
**보안 모드** (메뉴바에서 켜고 끈다, 기본 켜짐)는 사람이 보이는 동안 얼굴을 계속 확인한다. 등록되지 않은
얼굴이 또렷하게 다섯 번 연속 잡히면 10초를 기다리지 않고 바로 화면을 까맣게 하고 잠근다 (사용자 요청
2026-09-16). 등록된 얼굴이 보이면 횟수가 0으로 돌아가고, 얼굴이 안 보이거나 너무 멀거나(화면 높이의 8% 미만)
방이 어두우면 잠그지 않는다. 확인 주기는 등록된 얼굴이 보일 때 1초, 안 맞기 시작하면 0.25초라 보통 2초 안에
판정한다. Touch ID·암호로 푼 뒤 2분 동안은 얼굴로 다시 잠그지 않는다 — 카메라가 그 사람을 못 알아보는
상황이니 곧바로 다시 잠그면 갇히기 때문이다. 잠금이 꺼져 있거나 등록된 얼굴이 없으면 보안 모드도 동작하지
않고, 꺼 두면 얼굴 확인 자체를 돌리지 않는다.
