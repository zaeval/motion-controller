...
**손 주변만 잘라서 인식한다.** 작은 손이 통째 프레임에서는 잘 안 잡힌다 — 측정값으로, 화면 높이의 0.15만 한
검지 손은 프레임의 **35~40%에서 아예 안 잡혔고**, 0.26만 한 손바닥은 **하나도 놓치지 않았다**. Vision은
주어진 영역을 포즈 모델 입력 크기로 늘려서 보므로, 직전 프레임의 손 주변(양손 전체를 감싸는 박스의 3배,
픽셀 기준 정사각형, 최소 화면 높이의 30%)만 `regionOfInterest`로 주면 같은 손이 더 크게 들어간다. 크롭에서
손을 놓치면 그 프레임만 전체 화면으로 한 번 더 본다.

포즈 요청의 좌표는 크롭 기준으로 돌아오므로 전체 이미지 좌표로 되돌리는데, 혹시 그 규약이 다르면 커서가
엉뚱한 곳으로 갈 수 있다. 그래서 크롭으로 잰 손 크기가 직전 전체 화면 측정치와 2배 이상 어긋나면 **스스로
크롭을 끄고** 로그에 남긴다 (`Cropped hand measured ... the region-of-interest mapping is wrong`).
