...
(되돌아오는 손). 멈춤 없이 움직이는 손, 주먹 뒤로 빼기, 손등이 보이는 손은 전환하지 않는다. 오른쪽으로 쓸면
다음 데스크톱.
...
1. **얼굴** — 등록한 얼굴이 카메라에 1초 안팎으로 여러 번 확인되면 저절로 풀린다.
...
처음 실행하면 얼굴 등록 창이 한 번 뜨고, 등록을 마치면 잠금이 켜진다. 메뉴바 > **까만 화면 잠금** 으로
끄고, **얼굴 다시 등록…** 으로 바꾼다. 사진은 저장하지 않고, 얼굴에서 뽑은 512차원 숫자 벡터만
`~/Library/Application Support/MotionController/owner-face.json`에 저장한다. 그 파일을 지우면 등록이 없어진다.

잠긴 동안 누군가 키보드·마우스·트랙패드를 건드리면 카메라 사진을 찍는다. 그 순간 한 장, 이후 주인이 아닌
얼굴이 보일 때마다 1초 간격으로 최대 두 장 더 찍는다. 15초 안에 주인이 얼굴이나 Touch ID·암호로 풀면 주인이었던
것으로 보고 버린다. 아무도 못 풀거나 인증 창이 잠긴 채 닫히면 저장한다. 저장한 뒤 30초 동안은 새로 찍지 않는다.
